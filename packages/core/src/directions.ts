 ...
 * Same failure shape as `MERMAN_UNTRUSTED_TYPES` in `validate.ts`, but it can't
 * be handled the same way: denylisting `flowchart` and `graph` would put the
 * two most common diagram types on the slow path, which is most of the fast
 * path's value. So the header is checked instead, and only a bad direction
 * defects.
 *

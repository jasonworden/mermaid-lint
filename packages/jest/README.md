...
[Jest](https://jestjs.io) adapter for [mermaid-lint](https://github.com/jasonworden/mermaid-lint). Turns every Mermaid diagram in your docs into a test — your suite fails if a diagram has a syntax error (or, with `strict`, a semantic warning like a self-loop or duplicate id). Uses the official `mermaid.parse()` API.
